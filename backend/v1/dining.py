...
dining = Blueprint("v1/dining", __name__)

@dining.route("/restaurants",methods=["GET"])
@jwt_required
def fetch_all_restaurants():
    ...
    return json(res_rows), 200

@dining.route("restaurants/<int:res_id>", methods=["GET"])
@jwt_required
def fetch_restaurant_by_id(res_id):
    ...
    res_row["images"]=json.load(res_row["images"])
    res_row["time_slots"]=json.load(res_row["time_slots"])
    res_row["tags"]=json.load(res_row["tags"])
